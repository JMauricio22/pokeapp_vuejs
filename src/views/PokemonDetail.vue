...
export default {
  ...
  created() {
    this.$store
      .dispatch('getPokemonByName', { name: this.$route.params.name })
      .then((resp) => {
        console.log(resp);
      });
  },
  computed: {
    ...
    loading() {
      return (
        Object.keys(this.$store.state.selectedItem).length > 0 &&
    ...
    tabItems() {
      ...
      pokemonInfo.forEach((item) => {
        if (item[0].includes('abilities') || item[0].includes('stats')) {
          tabs.push(item);
        } else if (item[0].includes('height') || item[0].includes('weight') || item[0].includes('moves')) {
          tabs[0][1].push(item);
        }
      });
